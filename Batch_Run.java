...
public class Batch_Run implements PlugIn { 
	...
	public void run(String arg) { //Method Belonging to PlugIn Implementation
		...
		if (myDir1==null) return;									//If not source folder is selected the plugin stops here
		...
		if (myDir2==null) return;									//If not saving folder is selected the plugin stops here
		IJ.log("The saving folder chosen was"+myDir2);
			
		int Dzuzio;

		
		for (Dzuzio = 0; Dzuzio < myListSources.length; Dzuzio++) {
			
			IJ.showProgress(Dzuzio,myListSources.length);					//This command sets magnitude the progress bar on ImageJ
			IJ.log((Dzuzio+1)+":"+myListSources[Dzuzio]);
			IJ.showStatus(Dzuzio+"/"+myListSources.length);				//This command sets a text in the ImageJ status bar

			ImagePlus myImPlus = IJ.openImage(myDir1+myListSources[Dzuzio]);

			Opener o = new Opener ();

			int fileType = o.getFileType(myDir1+myListSources[Dzuzio]);
			if ( fileType==16) {
