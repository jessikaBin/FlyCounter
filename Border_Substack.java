...
public class Border_Substack implements PlugInFilter {
	...
	private double startingThreshold = 140.0; // starting threshold for border
												// detection
	// private double maxThreshold = Batch_Run.flyThreshold; // threshold for
	// detecting the flies
	private boolean mov = Batch_Run.mov;
	private boolean det = Batch_Run.det;

	// protected TreeMap <Integer, Integer> summaryWater = new TreeMap <Integer,
	// Integer> ();
	// protected TreeMap <Integer, Integer> summarySugar = new TreeMap <Integer,
	// Integer> ();
	...
	public void run(ImageProcessor ip) {
		...
		imp.killRoi();
		// substack (ip, imp, curr, stackSize); // create substack from current
		// slice until end of stack
	...
	private double setThresh(double startingThreshold, ImageProcessor ip,
		...
		if (rt.getCounter() == 0) {
			// if (test == 3) {
			// int curr = imp.getCurrentSlice(); // starting frame is changed to
			// current slice
			// imp.setSlice(curr+1);
			// test = 2;
			startingThreshold++;
			...
			double a = Double.parseDouble(splitt[1]); // circularity values are
														// in second column
	...
	public void substack(ImageProcessor ip, ImagePlus imp, int curr,
			int stackSize) {
	...
	public void detectFlies(ImageProcessor ip, ImagePlus imp, int curr) {
		...
		for (int i = curr; i <= stackSize; i++) {
			...
			countFlies(ip, imp, i, preferenceIndex); // applying setThresh
														// method
	...
	// ArrayList <TreeMap <Integer, Integer>> areas = new ArrayList <TreeMap
	// <Integer, Integer>> ();
	...
	private void countFlies(ImageProcessor ip, ImagePlus imp, int i,
		...
		if (det == true) {
			...
			// measurements: area & circularity & slice, options: show nothing,
			// minSize: flies not smaller than 8, maxSize: flies not bigger than
			// 150, minCirc/maxCirc: no defined circularity
