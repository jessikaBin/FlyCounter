...
/**
 * PlugIn Avi_Writer_WD modified the original AVI_Writer plugin of ImageJ 
 * (http://rsbweb.nih.gov/ij/plugins/download/AVI_Writer.java), so that no dialog window 
 * for the user comes up. Instead the current video and the chosen output folder from Batch_Run.java is used.
 * 
 */
 ...
 */
public class AVI_Writer_WD implements PlugInFilter {
	...
	public void writeImage(ImagePlus imp, String path, String filename)
			throws IOException {
		long saveFileSize; // location of file size in bytes not counting first
							// 8 bytes
		long saveLIST1Size; // location of length of CHUNK with first LIST - not
							// including
							// first 8 bytes with LIST and size. JUNK follows
							// the end of
							// this CHUNK
		int[] extents;
		long saveLIST1subSize; // location of length of CHUNK with second LIST -
								// not including
								// first 8 bytes with LIST and size. Note that
								// saveLIST1subSize =
								// saveLIST1Size + 76, and that the length size
								// written to
								// saveLIST2Size is 76 less than that written to
								// saveLIST1Size.
								// JUNK follows the end of this CHUNK.
		long savestrfSize; // location of lenght of strf CHUNK - not including
							// the first
							// 8 bytes with strf and size. strn follows the end
							// of this
							// CHUNK.
		int resXUnit = 0;
		int resYUnit = 0;
		...
		// SaveDialog sd = new SaveDialog("Save as AVI...", imp.getTitle(),
		// ".avi");
		...
		writeInt(0); // dwMaxBytesPerSec
						// Write the maximum data rate of the file in bytes per
						// second
		...
		writeInt(0x10); // dwFlags - just set the bit for AVIF_HASINDEX
						...
						// containing
						// an index at the end of the file. For good
						// performance, all
						// AVI files should contain an index.
						// 20H AVIF_MUSTUSEINDEX: Index CHUNK, rather than the
						// physical
						// ordering of the chunks in the file, must be used to
						// determine the
						// order of the frames.
						// 100H AVIF_ISINTERLEAVED: Indicates that the AVI file
						// is interleaved.
						// This is used to read data from a CD-ROM more
						// efficiently.
						// 800H AVIF_TRUSTCKTYPE: USE CKType to find key frames
						// 10000H AVIF_WASCAPTUREFILE: The AVI file is used for
						// capturing
						// real-time video. Applications should warn the user
						// before
						// writing over a file with this fla set because the
						// user
						// probably defragmented this file.
						// 20000H AVIF_COPYRIGHTED: The AVI file contains
						// copyrighted data
						// and software. When, this flag is used, software
						// should not
		...
		writeInt(0); // dwSuggestedBufferSize - Suggested buffer size for
						// reading the file.
						// Generally, this size should be large enough to
						// contain the largest
						// chunk in the file.
		...
		writeString("DIB ");
		writeInt(0); // dwFlags
						// 0x00000001 AVISF_DISABLED The stram data should be
						// rendered only when
						// explicitly enabled.
						// 0x00010000 AVISF_VIDEO_PALCHANGES Indicates that a
						// palette change is included
						// in the AVI file. This flag warns the playback
						// software that it
						// will need to animate the palette.
		writeInt(0); // dwPriority - priority of a stream type. For example, in
						// a file with
						// multiple audio streams, the one with the highest
						// priority might be the
						// default one.
		writeInt(0); // dwInitialFrames - Specifies how far audio data is skewed
						// ahead of video
						// frames in interleaved files. Typically, this is about
						// 0.75 seconds. In
						// interleaved files specify the number of frames in the
						// file prior
						// to the initial frame of the AVI sequence.
						// Noninterleaved files should use zero.
		// rate/scale = samples/second
		writeInt(1); // dwScale
		writeInt((int) Animator.getFrameRate()); // dwRate - frame rate for
													// video streams
		writeInt(0); // dwStart - this field is usually set to zero
		writeInt(tDim * zDim); // dwLength - playing time of AVI file as defined
								// by scale and rate
								// Set equal to the number of frames
		writeInt(0); // dwSuggestedBufferSize - Suggested buffer size for
						// reading the stream.
						// Typically, this contains a value corresponding to the
						// largest chunk
						// in a stream.
		...
		writeInt(0); // biClrImportant - specifies that the first x colors of
						// the color table
						// are important to the DIB. If the rest of the colors
						// are not available,
						// the image still retains its meaning in an acceptable
						// manner. When this
						// field is set to zero, all the colors are important,
						// or, rather, their
