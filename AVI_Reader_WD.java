...
/**
 * PlugIn AVI_Reader_WD modified the original ImageJ PlugIn AVI_Reader
 * (http://rsbweb.nih.gov/ij/plugins/download/AVI_Reader.java), so that no
 * dialog window is opened.
 * Thus, the input folder is the chosen folder of Batch_Run.java.
 * Furthermore no input for the parameters firstFrame, lastFrame, convertToGray,
 * flipVertical and isVirtual is asked put preset.
 * firstFrame = 1;      //the first frame to read
 * lastFrame = 0;       //the last frame to read; 0 means 'read all'
 * convertToGray = true;       //whether to convert color video to grayscale
 * flipVertical = false;        //whether to flip image vertical
 * isVirtual = true;		// read in video as virtual frames
 * 
 */
...
public class AVI_Reader_WD extends VirtualStack implements PlugIn {
	...
	private final static int FOURCC_idx1 = 0x31786469; // 'idx1' // index of
														// single 'movi' block
	...
	private final static int NO_COMPRESSION = 0; // uncompressed, also 'RGB ',
													// 'RAW '
	private final static int NO_COMPRESSION_RGB = 0x20424752; // 'RGB ' -a name
																// for
																// uncompressed
	private final static int NO_COMPRESSION_RAW = 0x20574152; // 'RAW ' -a name
																// for
																// uncompressed
	private final static int NO_COMPRESSION_Y800 = 0x30303859;// 'Y800' -a name
																// for 8-bit
																// grayscale
	private final static int NO_COMPRESSION_Y8 = 0x20203859; // 'Y8 ' -another
																// name for Y800
	private final static int NO_COMPRESSION_GREY = 0x59455247;// 'GREY' -another
																// name for Y800
	private final static int NO_COMPRESSION_Y16 = 0x20363159; // 'Y16 ' -a name
																// for 16-bit
																// uncompressed
																// grayscale
	private final static int NO_COMPRESSION_MIL = 0x204c494d; // 'MIL ' - Matrox
																// Imaging
																// Library
	private final static int AYUV_COMPRESSION = 0x56555941; // 'AYUV'
															...
															// but alpha, Y, U,
															// V bytes
	private final static int UYVY_COMPRESSION = 0x59565955; // 'UYVY' - 4:2:2
															// with byte order u
															// y0 v y1
	private final static int Y422_COMPRESSION = 0x564E5955; // 'Y422' -another
															// name for of UYVY
	private final static int UYNV_COMPRESSION = 0x32323459; // 'UYNV' -another
															// name for of UYVY
	private final static int CYUV_COMPRESSION = 0x76757963; // 'cyuv' -as UYVY
															// but not top-down
	private final static int V422_COMPRESSION = 0x32323456; // 'V422' -as UYVY
															// but not top-down
	private final static int YUY2_COMPRESSION = 0x32595559; // 'YUY2' - 4:2:2
															// with byte order
															// y0 u y1 v
	private final static int YUNV_COMPRESSION = 0x564E5559; // 'YUNV' -another
															// name for YUY2
	private final static int YUYV_COMPRESSION = 0x56595559; // 'YUYV' -another
															// name for YUY2
	private final static int YVYU_COMPRESSION = 0x55595659; // 'YVYU' - 4:2:2
															// with byte order
															// y0 u y1 v
	private final static int JPEG_COMPRESSION = 0x6765706a; // 'jpeg' JPEG
															// compression of
															// individual frames
	private final static int JPEG_COMPRESSION2 = 0x4745504a; // 'JPEG' JPEG
																// compression
																// of individual
																// frames
	private final static int JPEG_COMPRESSION3 = 0x04; // BI_JPEG: JPEG
														// compression of
														// individual frames
	private final static int MJPG_COMPRESSION = 0x47504a4d; // 'MJPG' Motion
															// JPEG, also reads
															// compression of
															// individual frames
	private final static int PNG_COMPRESSION = 0x20676e70; // 'png ' PNG
															// compression of
															// individual frames
	private final static int PNG_COMPRESSION2 = 0x20474e50; // 'PNG ' PNG
															// compression of
															// individual frames
	private final static int PNG_COMPRESSION3 = 0x05; // BI_PNG PNG compression
														// of individual frames

	private final static int BITMASK24 = 0x10000; // for 24-bit (in contrast to
													// 8, 16,... not a bitmask)
	...
	private final static int AVIF_HASINDEX = 0x00000010; // Index at end of
															// file?
	private final static int AVIF_MUSTUSEINDEX = 0x00000020; // ignored by this
																// plugin
	private final static int AVIF_ISINTERLEAVED = 0x00000100; // ignored by this
																// plugin
	...
	private final static byte AVI_INDEX_OF_INDEXES = 0x00; // main indx pointing
															// to ix00 etc
															// subindices
	...
	private boolean convertToGray = true; // whether to convert color video to
											// grayscale
	...
	private int type0xdb, type0xdc; // video stream chunks must have one of
									// these two types (e.g. '00db' for straem
									// 0)
	...
	private int frameNumber = 1; // frame currently read; global because
									// distributed over 1st AVi and further RIFF
									// AVIX chunks
	private int lastFrameToRead = Integer.MAX_VALUE;
	private int totalFramesFromIndex;// number of frames from 'AVI 2.0' indices
	private boolean indexForCountingOnly;// don't read the index, only count int
											// totalFramesFromIndex how many
											// entries
	...
	private boolean variableLength; // compressed (PNG, JPEG) frames have
									// variable length
	// for conversion to ImageJ stack
	private Vector frameInfos; // for virtual stack: long[] with frame pos&size
								// in file, time(usec)
	...
	private int dwFlags;
	private int dwTotalFrames; // AVI 2.0: will be replaced by number of frames
								// from index
	...
	private int dwPriorityLanguage; // actually 2 16-bit words: wPriority and
									// wLanguage
	...
	private short biPlanes; // no. of color planes: for the formats decoded;
							// here always 1
	private short biBitCount; // Bits per Pixel
	private int biCompression;
	private int biSizeImage; // size of image in bytes (may be 0: if so,
								// calculate)
	private int biXPelsPerMeter; // horizontal resolution, pixels/meter (may be
									// 0)
	...
	private int biClrImportant; // no. of important colors (appear first in
								// palette) (0 means all are important)

	/**
	 * The plugin is invoked by ImageJ using this method. String 'arg' may be
	 * used to select the path.
	 */
	public void run(String arg) {
		// OpenDialog od = new OpenDialog("Select AVI File", arg); //file dialog
		String fileName = Batch_Run.filename + ".avi";
		if (fileName == null)
			return;
		String fileDir = Batch_Run.myDir1;
		...
		if (stack == null || aborting
				|| (stack.isVirtual() && stack.getProcessor(1) == null))
			return;
		if (stack.getSize() == 0) {
			String rangeText = "";
	 ...
	 * @param lastFrame
	 *            Number of last frame to read or 0 for reading all, -1 for all
	 *            but last...
	 * @param isVirtual
	 *            Whether to return a virtual stack
	 * @param convertToGray
	 *            Whether to convert color images to grayscale
	 * @return Returns the stack; null on failure. The stack returned may be
	...
	public ImageStack makeStack(String path, int firstFrame, int lastFrame,
		...
		try {
			...
			stack.trim();
			IJ.showMessage("AVI Reader", "Out of memory.  " + stack.getSize()
					+ " of " + dwTotalFrames + " frames will be opened.");
		} catch (Exception e) {
	 ...
	 * (if it is one) where 1<=n<=nslices. Returns null if no virtual stack or
	 * no slices.
	 */
	...
	public String getSliceLabel(int n) {
		if (frameInfos == null || n < 1 || n > frameInfos.size())
			throw new IllegalArgumentException(
					"No Virtual Stack or argument out of range: " + n);
		return frameLabel(((long[]) (frameInfos.get(n - 1)))[2]);
	}

	/**
	 * Deletes the specified image from this virtual stack (if it is one), where
	 * 1<=n<=nslices.
	 */
	...
	private void readAVI(String path) throws Exception, IOException {
		...
		startTime += System.currentTimeMillis();// taking previously elapsed
												// time into account
		/** MOVED UP HERE BY JSP */
		if (lastFrame > 0) // last frame number to read: from Dialog
			lastFrameToRead = lastFrame;
		if (lastFrame < 0 && dwTotalFrames > 0) // negative means
												// "end frame minus ..."
		...
		if (isVirtual || firstFrame > 1) { // avoid scanning frame-by-frame
											// where we only need the positions
			frameInfos = new Vector(100); // holds frame positions, sizes and
											// time since start
			long nextPosition = -1;
			if (indexPosition > 0) { // attempt to get AVI2.0 index instead of
										// scanning for all frames
				raFile.seek(indexPosition);
				nextPosition = findFourccAndRead(FOURCC_indx, false,
						indexPositionEnd, false);
			}
			if (hasIndex && frameInfos.size() == 0) { // got nothing from indx,
														// attempt to read AVI 1
														// index 'idx1'
				raFile.seek(headerPositionEnd);
				moviPosition = findFourccAndSkip(FOURCC_movi, true, fileSize); // go
																				// behind
																				// the
																				// 'movi'
																				// list
				if (moviPosition > 0)
					nextPosition = findFourccAndRead(FOURCC_idx1, false,
							fileSize, false);
			}
			if (verbose)
				IJ.log("'frameInfos' has " + frameInfos.size() + " entries");
		}
		if (isVirtual && frameInfos.size() > 0) // Virtual Stack only needs
												// reading the index
			return;
		...
		if (firstFrame > 1 && frameInfos.size() > 0) {
			...
			raFile.seek(frameInfo[0] - 8); // chunk starts 8 bytes before frame
											// data
			frameNumber = firstFrame;
			if (verbose)
				IJ.log("directly go to frame " + firstFrame + " @ 0x"
						+ Long.toHexString(frameInfo[0] - 8));
	...
	private void openAndReadHeader(String path) throws Exception, IOException {
		...
		if (verbose)
			IJ.log("File header: File type='" + fourccString(fileType)
					+ "' (should be 'RIFF')" + timeString());
		...
		if (verbose)
			IJ.log("File header: RIFF type='" + fourccString(riffType)
					+ "' (should be 'AVI ')");
	...
	/**
	 * Read AVIX for large files (sequential reading frame-by-frame beyond the
	 * first Chunk)
	 **/
	 ...
	 * Returns next position If not found, throws exception or returns -1
	 */
	private long findFourccAndRead(int fourcc, boolean isList,
			long endPosition, boolean throwNotFoundException) throws Exception,
			IOException {
		long nextPos;
		...
		do {
			...
			if (type == 0) { // reached endPosition without finding
				if (throwNotFoundException)
					throw new Exception("Required item '"
							+ fourccString(fourcc) + "' not found");
				else
					return -1;
			}
			...
			if (isList && type == FOURCC_LIST)
				type = readInt();
			if (verbose)
				IJ.log("Searching for '" + fourccString(fourcc) + "', found "
						+ fourccString(type) + "' "
						+ posSizeString(nextPos - size, size));
			if (type == fourcc) {
				contentOk = readContents(fourcc, nextPos);
			} else if (verbose)
				IJ.log("Discarded '" + fourccString(type)
						+ "': Contents does not fit");
			raFile.seek(nextPos);
			if (contentOk)
	...
	/**
	 * Find the next position of fourcc or LIST fourcc, but does not read it,
	 * only returns the first position inside the fourcc chunk and puts the file
	 * pointer behind the fourcc chunk (if successful) If not found, returns -1
	 */
	private long findFourccAndSkip(int fourcc, boolean isList, long endPosition)
			throws IOException {
		while (true) {
			int type = readType(endPosition);
			if (type == 0) // reached endPosition without finding
			...
			if (isList && type == FOURCC_LIST)
				type = readInt();
			if (verbose)
				IJ.log("Searching for (to skip) '" + fourccString(fourcc)
						+ "', found " + fourccString(type) + "' "
	...
	private boolean readContents(int fourcc, long endPosition)
		...
		case FOURCC_avih:
			...
			return true;
		case FOURCC_strl:
			long nextPosition = findFourccAndRead(FOURCC_strh, false,
					endPosition, false);
			if (nextPosition < 0)
				return false;
			indexPosition = findFourccAndRead(FOURCC_strf, false, endPosition,
					true);
			indexPositionEnd = endPosition;
			indexForCountingOnly = true; // try reading indx for counting number
											// of entries
			totalFramesFromIndex = 0;
			nextPosition = findFourccAndRead(FOURCC_indx, false, endPosition,
					false);
		...
		case FOURCC_strh:
			...
			if (streamType != FOURCC_vids) {
				if (verbose)
					IJ.log("Non-video Stream '" + fourccString(streamType)
							+ " skipped");
				streamNumber++;
				return false;
			}
	...
	void readAviHeader() throws Exception, IOException { // 'avih'
		...
		dwReserved1 = readInt(); // in newer avi formats, this is
	...
	void readStreamHeader() throws Exception, IOException { // 'strh'
		...
		if (verbose) {
			...
			IJ.log("   dwStreamSuggestedBufferSize="
		...
		if (dwStreamSampleSize > 1)
			throw new Exception("Video stream with " + dwStreamSampleSize
					+ " (more than 1) frames/chunk not supported");
		// what the chunks in that stream will be named (we have two
		// possibilites: uncompressed & compressed)
		type0xdb = FOURCC_00db + (streamNumber << 8); // '01db' for stream 1,
														// etc. (inverse byte
														// order!)
		type0xdc = FOURCC_00dc + (streamNumber << 8); // '01dc' for stream 1,
														// etc.
	}

	/**
	 * Read 'AVI 2'-type main index 'indx' or an 'ix00' index to frames (only
	 * the types AVI_INDEX_OF_INDEXES and AVI_INDEX_OF_CHUNKS are supported)
	 */
	private void readAvi2Index(long endPosition) throws Exception, IOException {
		...
		if (verbose) {
			String bIndexString = bIndexType == AVI_INDEX_OF_CHUNKS ? ": AVI_INDEX_OF_CHUNKS"
					: bIndexType == AVI_INDEX_OF_INDEXES ? ": AVI_INDEX_OF_INDEXES"
							: ": UNSUPPOERTED";
			IJ.log("AVI 2 INDEX:");
			...
			if (bIndexType == AVI_INDEX_OF_CHUNKS)
				IJ.log("   qwBaseOffset=" + "0x"
						+ Long.toHexString(qwBaseOffset));
		}
		if (bIndexType == AVI_INDEX_OF_INDEXES) { // 'indx' points to other
													// indices
			if (wLongsPerEntry != 4)
			...
			for (int i = 0; i < nEntriesInUse; i++) {
				...
				if (verbose) {
					...
					IJ.log("   index data '" + fourccString(dwChunkId) + "' "
			...
			if (wLongsPerEntry != 2)
				return; // badly formed index, ignore it
			if (dwChunkId != type0xdb && dwChunkId != type0xdc) { // not the
																	// stream we
																	// search
																	// for?
																	// (should
				...
				if (verbose)
					IJ.log("INDEX ERROR: SKIPPED ix00, wrong stream number or type, should be "
							+ fourccString(type0xdb)
							+ " or "
							+ fourccString(type0xdc));
				return;
			}
			if (indexForCountingOnly) { // only count number of entries, don't
										// put into table
			...
			for (int i = 0; i < nEntriesInUse; i++) {
				...
				if (frameNumber >= firstFrame) {
					frameInfos.add(new long[] { pos, dwSize,
							(long) frameNumber * dwMicroSecPerFrame });
					// if (verbose)
	...
	private void readOldFrameIndex(long endPosition) throws Exception,
			IOException {
		int offset = -1; // difference between absolute frame address and
							// address given in idx1
		int[] offsetsToTry = new int[] { 0, (int) moviPosition }; // dwOffset
																	// may be
																	// w.r.t.
																	// file
																	// start or
			...
			if ((dwChunkId == type0xdb || dwChunkId == type0xdc) && dwSize > 0) {
				if (offset < 0) { // find out what the offset refers to
					...
					for (int i = 0; i < offsetsToTry.length; i++) {
						...
						raFile.seek(pos);
						int chunkIdAtPos = readInt(); // see whether this offset
					...
					if (verbose)
						IJ.log("idx1: dwOffsets are w.r.t. 0x"
								+ (offset < 0 ? " UNKONWN??" : Long
										.toHexString(offset)));
					raFile.seek(temp);
					if (offset < 0)
				...
				if (frameNumber >= firstFrame) {
					...
					frameInfos.add(new long[] { framepos + 8, dwSize,
							(long) frameNumber * dwMicroSecPerFrame });
					// if (verbose)
	...
	void readBitMapInfo(long endPosition) throws Exception, IOException {
		...
		if (verbose) {
			...
			IJ.log("   biCompression=0x" + Integer.toHexString(biCompression)
		...
		case NO_COMPRESSION_RAW:
			...
			dataTopDown = biHeight < 0; // RGB mode is usually bottom-up,
										// negative height signals top-down
			allowedBitCount = 8 | BITMASK24 | 32; // we don't support 1, 2 and 4
													// byte data
		...
		default:
			throw new Exception("Unsupported compression: "
					...
					+ (biCompression >= 0x20202020 ? " '"
							+ fourccString(biCompression) + "'" : ""));
		}

		int bitCountTest = biBitCount == 24 ? BITMASK24 : biBitCount; // convert
																		// "24"
																		// to a
																		// flag
		if (allowedBitCount != 0 && (bitCountTest & allowedBitCount) == 0)
			throw new Exception("Unsupported: " + biBitCount
					+ " bits/pixel for compression '"
					+ fourccString(biCompression) + "'");
		...
		if (verbose) {
			IJ.log("   > data compression=0x"
					+ Integer.toHexString(dataCompression) + " '"
		...
		if (readPalette) {
			...
			if (verbose)
				IJ.log("   Reading " + biClrUsed + " Palette colors: "
						+ posSizeString(spaceForPalette));
			if (spaceForPalette < biClrUsed * 4)
				throw new Exception("Not enough data (" + spaceForPalette
						+ ") for palette of size " + (biClrUsed * 4));
	...
	void readMovieData(long endPosition) throws Exception, IOException {
		if (verbose)
			IJ.log("MOVIE DATA "
					+ posSizeString(endPosition - raFile.getFilePointer())
					+ timeString());
		if (verbose)
		...
		if (isVirtual) {
			if (frameInfos == null) // we might have it already from reading the
									// first chunk
				frameInfos = new Vector(100); // holds frame positions in file
												// (for non-constant frame
												// sizes, should hold long[]
												// with pos and size)
		} else if (stack == null)
		...
		while (true) { // loop over all chunks
			...
			if ((type == type0xdb || type == type0xdc) && size > 0) {
				IJ.showProgress((double) frameNumber / lastFrameToRead);
				if (verbose)
					IJ.log(frameNumber + " movie data '" + fourccString(type)
							+ "' " + posSizeString(size) + timeString());
				if (frameNumber >= firstFrame) {
					if (isVirtual)
						frameInfos.add(new long[] { pos, size,
								frameNumber * dwMicroSecPerFrame });
					else { // read the frame
						Object pixels = readFrame(raFile, pos, (int) size);
						String label = frameLabel(frameNumber
								* dwMicroSecPerFrame);
				...
				IJ.log("skipped '" + fourccString(type) + "' "
						+ posSizeString(size));
	...
	/**
	 * Reads a JPEG or PNG-compressed frame from a RandomAccessFile and returns
	 * the pixels array of the resulting image abd sets the ColorModel cm (if
	 * appropriate)
	 */
	private Object readCompressedFrame(RandomAccessFile rFile, int size)
			throws Exception, IOException {
		InputStream inputStream = new raInputStream(rFile, size,
				biCompression == MJPG_COMPRESSION);
	...
	/**
	 * Read a fixed-length frame (RandomAccessFile rFile, long filePos, int
	 * size) return the pixels array of the resulting image
	 */
	private Object readFixedLengthFrame(RandomAccessFile rFile, int size)
			throws Exception, IOException {
		if (size < scanLineSize * biHeight) // check minimum size (fixed frame
											// length format)
			throw new Exception("Data chunk size " + size + " too short ("
					+ (scanLineSize * biHeight) + " required)");
	...
	/**
	 * For one line: copy byte data into the byte array for creating a
	 * ByteProcessor
	 */
	void unpack8bit(byte[] rawData, int rawOffset, byte[] pixels,
	 ...
	 */
	void unpackGray(byte[] rawData, int rawOffset, byte[] pixels,
			int byteOffset, int w) {
		int j = byteOffset;
		int k = rawOffset;
		if (dataCompression == 0) {
			...
			if (dataCompression == UYVY_COMPRESSION
					|| dataCompression == AYUV_COMPRESSION)
	 ...
	 * ShortProcessor
	 */
	void unpackShort(byte[] rawData, int rawOffset, short[] pixels,
			int shortOffset, int w) {
		int j = shortOffset;
		int k = rawOffset;
		for (int i = 0; i < w; i++) {
	...
	/**
	 * For one line: Read YUV, RGB or RGB+alpha data and writes RGB int array
	 * for ColorProcessor
	 */
	void unpack(byte[] rawData, int rawOffset, int[] pixels, int intOffset,
			int w) {
		int j = intOffset;
		int k = rawOffset;
		...
		case YUY2_COMPRESSION:
			for (int i = 0; i < w / 2; i++) {
				...
				int u = rawData[k++] ^ 0xffffff80; // converts byte range 0...ff
													// to -128 ... 127
	...
	/**
	 * Write an intData RGB value converted from YUV, The y range between 16 and
	 * 235 becomes 0...255 u, v should be between -112 and +112
	 */
	final void writeRGBfromYUV(int y, int u, int v, int[] pixels,
			int intArrayIndex) {
	...
	/**
	 * Read 8-byte int with Intel (little-endian) byte order (note:
	 * RandomAccessFile.readLong has other byte order than AVI)
	 */
	...
	/**
	 * Read 4-byte int with Intel (little-endian) byte order (note:
	 * RandomAccessFile.readInt has other byte order than AVI)
	 */
	...
	/**
	 * Read 2-byte short with Intel (little-endian) byte order (note:
	 * RandomAccessFile.readShort has other byte order than AVI)
	 */
	...
	/**
	 * Read type of next chunk that is not JUNK. Returns type (or 0 if no
	 * non-JUNK chunk until endPosition)
	 */
	...
	private void setFramesPerSecond(ImagePlus imp) {
		if (dwMicroSecPerFrame < 1000 && dwStreamRate > 0) // if no reasonable
															// frame time, get
															// it from rate
	...
	private String posSizeString(long pos, long size) throws IOException {
		return "0x" + Long.toHexString(pos) + "-0x"
				+ Long.toHexString(pos + size - 1) + " (" + size + " Bytes)";
	}
	...
	/**
	 * returns a string of a four-cc code corresponding to an int (Intel byte
	 * order)
	 */
	...
	private String exceptionMessage(Exception e) {
		String msg;
		if (e.getClass() == Exception.class) // for "home-built" exceptions:
												// message only
			msg = e.getMessage();
		else
			msg = e + "\n" + e.getStackTrace()[0] + "\n" + e.getStackTrace()[1];
	 ...
	 * position). This class also adds 'Define Huffman Table' (DHT) segments to
	 * convert MJPG to JPEG.
	 */
	final private static int BUFFERSIZE = 4096; // should be large enough to
												// hold the full JFIF header
	...
	final private static byte[] HUFFMAN_TABLES = new byte[] { // the 'DHT'
			...
			(byte) 0xA1, 0x08, 0x23, 0x42, (byte) 0xB1, (byte) 0xC1, 0x15,
			0x52, (byte) 0xD1, (byte) 0xF0, 0x24, 0x33, 0x62, 0x72,
			(byte) 0x82, 0x09, 0x0A, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x25, 0x26,
			...
			(byte) 0xF8, (byte) 0xF9, (byte) 0xFA, 0x11, 0x00, 0x02, 0x01,
			0x02, 0x04, 0x04, 0x03, 0x04, 0x07, 0x05, 0x04, 0x04, 0x00, 0x01,
			0x02, 0x77, 0x00, 0x01, 0x02, 0x03, 0x11, 0x04, 0x05, 0x21, 0x31,
			0x06, 0x12, 0x41, 0x51, 0x07, 0x61, 0x71, 0x13, 0x22, 0x32,
			(byte) 0x81, 0x08, 0x14, 0x42, (byte) 0x91, (byte) 0xA1,
			(byte) 0xB1, (byte) 0xC1, 0x09, 0x23, 0x33, 0x52, (byte) 0xF0,
			0x15, 0x62, 0x72, (byte) 0xD1, 0x0A, 0x16, 0x24, 0x34, (byte) 0xE1,
			0x25, (byte) 0xF1, 0x17, 0x18, 0x19, 0x1A, 0x26, 0x27, 0x28, 0x29,
			...
			0x75, 0x76, 0x77, 0x78, 0x79, 0x7A, (byte) 0x82, (byte) 0x83,
	...
	class raInputStream extends InputStream {
		...
		int readableSize; // number of bytes that one should expect to be
							// readable
		boolean fixMJPG; // whether to use an ugly hack to convert MJPG frames
							// to JPEG
		...
		raInputStream(RandomAccessFile rFile, int readableSize, boolean fixMJPG)
			...
			if (fixMJPG) {
				...
				bufferLength = Math.min(BUFFERSIZE - HUFFMAN_LENGTH,
						readableSize);
		...
		public int read() throws IOException {
			...
			if (fixMJPG) {
				...
				if (bufferPointer >= bufferLength)
					fixMJPG = false; // buffer exhausted, no more attempt to fix
										// it
				return result;
			} else
				return rFile.read();
		...
		private void addHuffmanTables() {
			...
			do {
				...
				if (code == 0xffc4) // Huffman table found, nothing to do
					return;
				else if (code == 0xffda || code == 0xffd9) { // start of image
																// data or end
																// of image?
				...
				offset += 2;
				segmentLength = readShort(offset); // read length of this
													// segment
		...
		private void insertHuffmanTables(int position) {
			...
			System.arraycopy(buffer, position, buffer, position
					+ HUFFMAN_LENGTH, bufferLength - position);
			System.arraycopy(HUFFMAN_TABLES, 0, buffer, position,
					HUFFMAN_LENGTH);
